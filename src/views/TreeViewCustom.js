...
const TreeView = () => {
  ...
  const prepareTreeData = () => {
    ...
    while(onQueue.length > 0){
      ...
      if(eachMember == 0) continue;
      traversed.push(eachMember);
      let memberData = {};

      memberData.id = eachMember;
      memberData.name = familyMembers[eachMember].name;
      memberData.gender = familyMembers[eachMember].gender;
      let itsPartner = getPartner(eachMember,relations,familyMembers);
      let itsParents = getParents(eachMember,relations,familyMembers);
      let itsChildren = getChildrensOfParents([eachMember,...itsPartner],relations,familyMembers);
      
      onQueue.push(...itsPartner,...itsParents,...itsChildren);

      let partners = []
      itsPartner.forEach(function(ele){
        let eachPartner = {
          id : familyMembers[ele].id,
          type : "married"
        }
        partners.push(eachPartner);
      })
      memberData.spouses = partners;

      let parents = []
      itsParents.forEach(function(ele){
        if(familyMembers[ele]){
          let eachParent = {
            id : familyMembers[ele].id,
            type : "blood"
          }
      ...
      memberData.parents = parents;

      let childrens = []
      itsChildren.forEach(function(ele){
        let eachChildren = {
          id : familyMembers[ele].id,
          type : "blood"
        }
        childrens.push(eachChildren);
      })
      memberData.children = childrens;

      if(itsParents[0] != 0 && itsParents[1] != 0){
        let itsSiblings = getChildrensOfParents(itsParents,relations,familyMembers);
        let siblings = []
        itsSiblings.forEach(function(ele){
          if(ele != eachMember){
            let eachSibling = {
              id : familyMembers[ele].id,
              type : "blood"
            }
            siblings.push(eachSibling);
          }
        })
        memberData.siblings = siblings;
